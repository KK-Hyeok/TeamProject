...
public class navscript : MonoBehaviour
{
   public float updateInterval = 3f; // 목표 위치를 갱신할 시간 간격 (초)
    ...
    {
        Vector3 randomDirection = Random.insideUnitSphere * 50f; // 원하는 범위 내의 랜덤한 방향 벡터를 생성합니다.
        randomDirection += transform.position; // 랜덤 방향 벡터를 현재 위치에 더합니다.

        NavMeshHit hit;
        if (NavMesh.SamplePosition(randomDirection, out hit, 50f, NavMesh.AllAreas)) // 랜덤 위치가 NavMesh 위에 있는지 확인합니다.
        {
